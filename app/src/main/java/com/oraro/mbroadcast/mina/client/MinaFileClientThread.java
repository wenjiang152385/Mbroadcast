...
public class MinaFileClientThread extends Thread {
    private static final String TAG = MinaFileClientThread.class.getSimpleName();
    private IoSession session = null;
    private static HashMap<String,IoConnector> connectorHashMap = new HashMap();
    private String ip;
    private PlayVO playVO = null;
    private static MinaFileClientThread mInstance;
    ...
    public static IoConnector getIoConnector(String IP) {
        if (connectorHashMap.get(IP)== null) {
            synchronized (MinaFileClientThread.class) {
                if (connectorHashMap.get(IP) == null) {
...
//		//设置默认连接远程服务器的IP地址和端口
                    ioConnector.setDefaultRemoteAddress(new InetSocketAddress(IP, Constants.WEB_MATCH_PORT));
     ...
     * @return
     */
    public static MinaFileClientThread getInstance() {
        if (mInstance == null) {
            synchronized (MinaFileClientThread.class) {
                if (mInstance == null) {
                    mInstance = new MinaFileClientThread();
                }
            }
        }
        return mInstance;
    ...
    public void run() {
        ...
        //开始连接
          IoConnector ioConnector=  getIoConnector(ip);
            LogUtils.e(TAG, "客户端链接开始..."+ioConnector.isActive());
            ...
            connectorHashMap.get(ip).setHandler(minaStringClientHandler);
