...
public class MinaStringClientThread extends Thread {
    private static final String TAG = MinaStringClientThread.class.getSimpleName();
    private IoSession session = null;
    private static HashMap<String,IoConnector> connectorHashMap = new HashMap();
    private String ip;
    private PlayVO playVO = null;
    private static MinaStringClientThread mInstance;
    ...
    public static IoConnector getIoConnector(String IP) {
        if (connectorHashMap.get(IP)== null) {
            synchronized (MinaStringClientThread.class) {
                if (connectorHashMap.get(IP) == null) {
...
//		//设置默认连接远程服务器的IP地址和端口
     ...
     * @return
     */
    public static MinaStringClientThread getInstance() {
        if (mInstance == null) {
            synchronized (MinaStringClientThread.class) {
                if (mInstance == null) {
                    mInstance = new MinaStringClientThread();
                }
            }
        }
        return mInstance;
